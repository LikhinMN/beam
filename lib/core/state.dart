...
typedef AppState = ({bool isServerRunning, List<TransferEvent> transfers});

/// Create the global Pico store with the initial state.
final store = Store<AppState>((isServerRunning: false, transfers: []));

/// Actions to mutate the state

void setServerRunning(bool isRunning) {
  store.set(
    (state) => (isServerRunning: isRunning, transfers: state.transfers),
  );
}
...
void clearTransfers() {
  store.set((state) => (isServerRunning: state.isServerRunning, transfers: []));
}
